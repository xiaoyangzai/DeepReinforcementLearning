...
class PolicyValueNet():
    def __init__(self,board_width,board_height,model_file = None):
        ...
        self.action_fc = tf.layers.dense(inputs=self.action_conv_flat,units=board_height*board*width,activation=tf.nn.log_softmax)
        ...
        self.evaluation_fc1 = tf.layers.dense(inputs=self.evaluation_conv_flat,units=64,actionvation=tf.nn.relu)
        #Evaluation output
        self.evaluation_fc2 = tf.layers.dense(inputs=self.evaluation_fc1,units=1,activation=tf.nn.tanh)

        #Define the loss function
        #1. Evaluation network labels(win or not)
        self.labels = tf.placeholder(tf.float32, shape=[None,1])
        #2. Value Loss function
        self.value_loss = tf.losses.mean_squared_error(self.labels,self.evaluation_fc2)

        #3. Policy Loss function
    ...
    def policy_value(self,state_batch):
        """
            ...
            output: a batch of action probalities and state values
        """
    ...
    def train_step(self,state_batch,mcts_probs,winner_batch,lr):
        ...
        loss, entropy,_ = self.session.run([self.loss,self.entropy,self.optimizer],feed_dict={self.input_states: state_batch, self.mcts_probs:mcts_probs,self.labels:winner_batch,self.learning_rate:lr})
        return loss,entropy

    def save_model(self,model_path):
        iself.saver.save(self.session,model_path)
