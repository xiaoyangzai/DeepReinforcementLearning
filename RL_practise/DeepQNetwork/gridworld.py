...
class GridWorldEnv(gym.Env):
    ...
    def __init__(self, n_width:int=10,
        ...
        self.action_name = {0:"left",1:"right",2:"up",3:"down"}
    ...
    def step(self, action):
        ...
        # 提供格子世界所有的信息在info内
        #info = "old x: %d, old_y: %d, action: %s , next x: %d,next y: %d"%(old_x,old_y,self.action_name[action],new_x,new_y)
        #info = [old_x,old_y,self.action_name[action],new_x,new_y]
        info = False
        if (new_x,new_y) == self.ends[0]:
            info = True
    ...
    def _xy_to_state(self, x, y = None):
        ...
        if isinstance(x, int) or isinstance(x,np.int64):
            ...
            temp_state = x + self.n_width * y
        elif isinstance(x, tuple):
            temp_state = x[0] + self.n_width * x[1]

        one_hot_state = np.zeros(self.observation_space.n)
        one_hot_state[temp_state] = 1
        self.state = one_hot_state 
        return self.state        # 未知状态
    ...
    def _is_end_state(self, x, y=None):
        if y is not None:
            xx, yy = x, y
        elif isinstance(x, int) or isinstance(x,np.int64):
